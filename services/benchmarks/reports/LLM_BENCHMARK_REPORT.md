...
- **Strict-JSON validation:** Implemented in `services/benchmarks/llm_benchmark.py::validate_format`. Checks (a) dict type, (b) required keys present, (c) `options` is a list of exactly 4 items, (d) `correct` ∈ {A,B,C,D}. Pass = 1.0, Fail = 0.0. No partial credit.
